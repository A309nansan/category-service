...
cd category || { echo "디렉토리 변경 실패"; exit 1; }
...
# 기존 서비스 삭제
log "category-service undeploy"
docker rm -f category-service

# 기존 category-service 이미지를 삭제하고 새로 빌드
log "category-service image remove and build."
docker rmi category-service:latest || true
docker build -t category-service:latest .

# Docker로 category-service 서비스 실행
log "Execute category-service..."
docker run -d \
